...
def bert_model():
    ...
    csv_obj = s3.get_object(Bucket=BUCKET, Key='location of s3 to store the data')
    body = csv_obj['Body']
    ...
    classifier_model.save('location to store the file', include_optimizer=False)
    ...
    s3_resource.Bucket(bucket_name).upload_file("local location", "s3 location") #change
